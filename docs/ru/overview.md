...
В первую очередь это касается процессов. Перед использованием каждого процесса должен быть определён его тип[^10], где указывается имя типа процесса, его приоритет и размер области ОЗУ, отведённой под [стек процесса](processes.md#processes-process-stack). Например:
