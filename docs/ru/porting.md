...
Ввиду больших отличий как аппаратных архитектур, так и средств разработки под них, возникает необходимость в специальной адаптации кода ОС[^porting-1] под них. Результатом этой работы является платформеннозависимая часть, которая в совокупности с общей частью и составляет порт под ту или иную платформу. Процесс подготовки платформеннозависимой части называется портированием.

[^porting-1]: Это касается не только ОС, но и других кроссплатформенных программ.
...
Платформеннозависимая часть каждой целевой платформы содержится в отдельной директории и минимально содержит три файла:
  ...
  * **os_target_asm.ext**[^porting-2] – низкоуровневый код, функции переключения контекста, старта ОС.
...
[^porting-2]: Расширение ассемблерного файла для целевого процессора.
...
Квалифицирует исполняемую функцию процесса. Содержит платформеннозависимый атрибут, указывающий компилятору, что функция не имеет возврата, поэтому preserved[^porting-4] регистры процессора можно использовать без сохранения. Это экономит код и пространство в стеке.

[^porting-4]: Те, значение которых перед использованием должно быть сохранено, а после использования восстановлено, чтобы вызывающая функция не получила искажения контекста при вызове другой функции.
...
Обработчик прерываний переключения контекстов. Реализуется на ассемблере. Производит сохранение контекста прерываемого процесса, переключение указателей стеков процессов путём вызова `context_switch_hook()`[^porting-5] и восстановление контекста активируемого процесса.

[^porting-5]: Через функцию-"обёртку"" `os_context_switch_hook()`, имеющую спецификацию связывания `"extern C"`.
...
Наиболее "тонкая" и ответственная работа при портировании выпадает на реализацию ассемблерного кода и на функцию подготовки стекового кадра. Ряд моментов, на которые следует обратить особое внимание:
  ...
  * проверить схему кодирования экспортируемых/импортируемых имён ассемблера. В простейшем случае имена объектов и функций на С (и `"extern C"`[^porting-6] на С++) на ассемблере видны без изменений, но на некоторых платформах[^porting-7] к самому имени могут добавляться префиксы и/или суффиксы, что потребует ассемблерные функции именовать в соответствии с этой схемой, иначе линкер не сможет правильно выполнить связи.
...
[^porting-7]: В частности, на Blackfin.
