...
* Verifying the name mangling scheme for exported/imported symbols in assembly. In the simplest case, C names (and `"extern C"`[^6] names in C++) are visible unchanged in assembly, but on some platforms[^7] prefixes and/or suffixes may be added, requiring assembly functions to be named accordingly for correct linker resolution.
